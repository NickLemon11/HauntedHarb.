...
void Crab::fire() {
	bullets.add(new Bullet("GreenFire.bmp", x + (facing_right ? w : 0), py + 10, 6, 6, ESPEED * 1.7 * (facing_right ? 1 : -1), 0));
}
...
void Crab::platform(list<Box*> boxes, list<Platform*> platforms) {
	for (Box* b : boxes)
		if (b->collides(this)) {
			...
			else if (px + hw < b->px) { x = b->x - hw - 1; xspeed *= -1; }
			else if (px > b->px + b->hw) { x = b->x + b->hw + 1; xspeed *= -1; }
