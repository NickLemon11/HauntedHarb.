...
COLORREF Button::lighten_color(COLORREF to_lighten) {
    return COLORREF(
        RGB(
            GetRValue(to_lighten) + 30 > 255 ? 255 : GetRValue(to_lighten) + 25,
            GetGValue(to_lighten) + 30 > 255 ? 255 : GetGValue(to_lighten) + 25,
            GetBValue(to_lighten) + 30 > 255 ? 255 : GetBValue(to_lighten) + 25,
...
COLORREF Button::darken_color(COLORREF to_darken) {
    return COLORREF(
        RGB(
            GetRValue(to_darken) - 30 < 0 ? 0 : GetRValue(to_darken) - 25,
            GetGValue(to_darken) - 30 < 0 ? 0 : GetGValue(to_darken) - 25,
            GetBValue(to_darken) - 30 < 0 ? 0 : GetBValue(to_darken) - 25,
        )
    );
}
